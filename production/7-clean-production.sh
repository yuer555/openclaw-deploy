#!/bin/bash
#==============================================================================
# 脚本名称: 7-clean-production.sh
# 功能描述: 清理生产环境（停止服务、删除容器、镜像、数据）
# 使用方法: ./7-clean-production.sh
# 执行位置: 服务器
# 版本: V2.0
#==============================================================================
...
echo -e "${RED}🧹 清理 OpenClaw 生产环境${NC}\n"
...
# 删除数据目录
read -p "是否删除数据目录？(输入 yes 删除): " delete_data
if [ "$delete_data" == "yes" ]; then
    print_info "删除数据目录..."
    rm -rf /opt/openclaw/data /opt/openclaw/shared-files /opt/openclaw/logs
    print_success "数据目录已删除"
fi

# 删除网络
docker network rm openclaw-prod-net 2>/dev/null || true
print_success "Docker 网络已删除"

# 删除 .env.prod（可选）
read -p "是否删除 .env.prod？(输入 yes 删除): " delete_env
if [ "$delete_env" == "yes" ]; then
    rm -f .env.prod
    print_success ".env.prod 已删除"
fi

echo ""
print_success "清理完成！重新部署：./3-deploy-production.sh"
echo ""
